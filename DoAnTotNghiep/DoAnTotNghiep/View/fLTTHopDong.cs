...
{
    public partial class fLTTHopDong : Form
    {
        ...
        public fLTTHopDong(HopDongMuaBanDTO HDMBXuLy)
        {
            InitializeComponent();
        ...
        private void fLTTHopDong_Load(object sender, EventArgs e)
        {
            txtID.Visible = false;
            DataTable dataLTT = HopDongMuaBanDAO.Instance.GetLichThanhToanByIDHDMB(this.HDMBXuLy.Id);

            dgvDanhMucLichThanhToan.DataSource = dataLTT;
            dgvDanhMucLichThanhToan.Columns["id"].HeaderText = "ID";

            dgvDanhMucLichThanhToan.Columns["idhopdongmuaban"].HeaderText = "ID Hợp Đồng Mua Bán";
        ...
        void Bingding()
        {
            ...
            txtIDHDMB.DataBindings.Add("Text", dgvDanhMucLichThanhToan.DataSource, "idhopdongmuaban", true, DataSourceUpdateMode.Never);
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            {
                // xoa
                if (HopDongMuaBanDAO.Instance.XoaLTTHopDong(int.Parse(txtID.Text)))
                {
        ...
        private void btnThucHien_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (flag == 1)
                {
                    ...
                    if (HopDongMuaBanDAO.Instance.ThemLTTHopDong(idhopdongmuaban, ngaythu, sotien, phantram, trangthai, anhphieuthu, ghichu))
                    {
                ...
                if (flag == 2)
                {
                    ...
                    double sotien = double.Parse(txtSoTien.Text);
                    double phantram = double.Parse(txtPhanTram.Text);
                    if (HopDongMuaBanDAO.Instance.SuaLTTHopDong(id, idhopdongmuaban, ngaythu, sotien, phantram, trangthai, anhphieuthu, ghichu))
                    {
        ...
        private void fLTTHopDong_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void txtSoTien_TextChanged(object sender, EventArgs e)
        {
            ...
            double sotien = double.Parse(txtSoTien.Text);
            double thanhtien = HopDongMuaBanDAO.Instance.GetThanhTien(this.HDMBXuLy.Id);
            double tmp = Math.Round(sotien / thanhtien * 100, 2);
